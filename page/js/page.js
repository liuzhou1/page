...
function page_ctrl(data_obj) {
  ...
  /* change_content(); */
      ...
      append_html+='<button class="next_page">下一页</button><span class="page_total">共 '+total_page+' 页  共'+total_item+'条,每页<select class="selected-box">\n' +
          '                <option value="10" >10</option>' +
          '                <option value="20" >20</option>' +
          '                <option value="50" >50</option>' +
          '                <option value="1000000000" >所有</option>' +
          '            </select>条, 到第</span><input class="input_page_num" type="text" value="'+inp_val+'"><span class="page_text">页</span><button class="to_page_num">确定</button>';
      $(obj_box).children('.page_ctrl').append(append_html);
      for (let i=0; i<$(".selected-box").find("option").length; i++){
        if ($(".selected-box").find("option").eq(i).val() == per_num){
          $(".selected-box").find("option").eq(i).attr("selected",true);
        }
      }
      if(current_page==1){
    ...
    $(obj_box+' .page_ctrl').on('change','select',function () {
      var that=$(this);
      per_num = that.children('option:selected').val();
      that.parents('.page').html('');
      var pageDate={
        obj_box:'.page',//翻页容器
        total_item:totalCount,//条目总数
        per_num: per_num,//每页条目数
        current_page:currentPage//当前页
      };
      pageSize = per_num;
      page_ctrl(pageDate);
      tableReload();
      console.log(that.children('option:selected').val());
    })
  }
